...
public class CourseController {
    ...
    public ResponseEntity<ApiResponseDto<List<CourseResponseDto>>> getAvailableCourses() {
        ...
        return ResponseEntity.ok(ApiResponseDto.success(courses, "Found " + courses.size() + " available courses"));
    }
    ...
    public ResponseEntity<ApiResponseDto<List<CourseResponseDto>>> getAllCourses() {
        ...
        return ResponseEntity.ok(ApiResponseDto.success(courses, "Found " + courses.size() + " courses"));
    }

    @GetMapping("/{id}")
    @Operation(summary = "Get course by ID")
    public ResponseEntity<ApiResponseDto<CourseResponseDto>> getCourseById(@PathVariable Long id) {
        return ResponseEntity.ok(ApiResponseDto.success(courseService.getCourseById(id), "Course found"));
    }
    ...
    public ResponseEntity<ApiResponseDto<CourseResponseDto>> createCourse(@Valid @RequestBody CourseRequestDto request) {
        ...
        return ResponseEntity.status(HttpStatus.CREATED).body(ApiResponseDto.success(created, "Course created successfully"));
    }
    ...
    public ResponseEntity<ApiResponseDto<CourseResponseDto>> updateCourse(@PathVariable Long id, @Valid @RequestBody CourseRequestDto request) {
        return ResponseEntity.ok(ApiResponseDto.success(courseService.updateCourse(id, request), "Course updated successfully"));
    }
    ...
    public ResponseEntity<ApiResponseDto<CourseResponseDto>> toggleAvailability(@PathVariable Long id) {
        ...
        return ResponseEntity.ok(ApiResponseDto.success(updated, "Course availability updated to: " + updated.getAvailable()));
    }

    @GetMapping("/category/{category}")
    @Operation(summary = "Get courses by category")
    public ResponseEntity<ApiResponseDto<List<CourseResponseDto>>> getCoursesByCategory(@PathVariable String category) {
        List<CourseResponseDto> courses = courseService.getCoursesByCategory(category);
        return ResponseEntity.ok(ApiResponseDto.success(courses, "Found " + courses.size() + " courses in category: " + category));
    }

    @GetMapping("/search")
    @Operation(summary = "Search courses by keyword")
    public ResponseEntity<ApiResponseDto<List<CourseResponseDto>>> searchCourses(@RequestParam String keyword) {
        List<CourseResponseDto> courses = courseService.searchCourses(keyword);
        return ResponseEntity.ok(ApiResponseDto.success(courses, "Found " + courses.size() + " courses matching: " + keyword));
    }

    @GetMapping("/filter/price")
    @Operation(summary = "Filter courses by price range")
    public ResponseEntity<ApiResponseDto<List<CourseResponseDto>>> getCoursesByPriceRange(@RequestParam BigDecimal min, @RequestParam BigDecimal max) {
        List<CourseResponseDto> courses = courseService.getCoursesByPriceRange(min, max);
        return ResponseEntity.ok(ApiResponseDto.success(courses, "Found " + courses.size() + " courses in price range"));
    }
    ...
    public ResponseEntity<ApiResponseDto<List<String>>> getAllCategories() {
        ...
        return ResponseEntity.ok(ApiResponseDto.success(categories, "Found " + categories.size() + " categories"));
    }
}

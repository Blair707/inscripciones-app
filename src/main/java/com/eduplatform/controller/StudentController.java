...
public class StudentController {
    ...
    public ResponseEntity<ApiResponseDto<Student>> registerStudent(@Valid @RequestBody StudentRequestDto request) {
        Student student = studentService.createOrGetStudent(request);
        return ResponseEntity.status(HttpStatus.CREATED).body(ApiResponseDto.success(student, "Student registered successfully"));
    }

    @GetMapping("/{id}")
    @Operation(summary = "Get student by ID")
    public ResponseEntity<ApiResponseDto<Student>> getStudentById(@PathVariable Long id) {
        return ResponseEntity.ok(ApiResponseDto.success(studentService.getStudentById(id), "Student found"));
    }
}
